...
//
// Every message is a single frame: a 4-byte big-endian length header followed by a JSON-encoded body. A client sends a
// Request and the server answers with exactly one Response.
package protocol
...
// ErrMessageTooLarge reports a body that does not fit in a frame. WriteMessage returns it before writing anything, so
// the caller may answer with something smaller on the same connection.
...
// Error codes classifying Response.Error. They let a client act on the reason a request failed without matching on the
// message, which stays free-form so that SQLite's own wording reaches the user unchanged.
const (
	CodeTimeout          = "timeout"
...
// AuthChallenge is the server's answer to a TypeAuthInit request: how to derive the salted password, and the nonce to
// bind the proof to.
